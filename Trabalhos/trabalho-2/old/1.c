...
void display(void)
{
   ...
   glPushMatrix();
	GLfloat sol[4] = {1.0f, 1.0f, 0.0f, 1.0f};
	glMaterialfv(GL_FRONT, GL_EMISSION, sol);
   ...
   //mercurio
   glPushMatrix();
	GLfloat mercurio[4] = {1.0f, 0.5f, 0.31f, 1.0f};
	glMaterialfv(GL_FRONT, GL_EMISSION, mercurio);
   ...
   //venus
	GLfloat cor[4] = {1.0f, 0.5f, 0.31f, 1.0f};
	glMaterialfv(GL_FRONT, GL_EMISSION, mercurio);
   glColor3f(1.0,0.98,0.804);
   glPushMatrix();
   glRotatef(spin, 1.0, 0.0, 0.0);
...
void reshape(int w, int h)
{
   ...
   glLoadIdentity();
   //gluLookAt(0.0,-1.0,0.0, 0.0,15000.0, 0.0,1.0,0.0,0.0); // visão superior
